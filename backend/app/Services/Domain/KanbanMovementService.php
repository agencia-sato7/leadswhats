...
class KanbanMovementService
{
    ...
    /**
     * @return array{lead_id:int,kanban_column_id:int,moved_by_user_id:int|null,movement_type:string,history_created:bool}
     */
    public function moveLeadToColumnByAi(int $companyId, int $leadId, int $kanbanColumnId, ?string $reason): array
    {
        $lead = Lead::query()
            ->where('company_id', $companyId)
            ->findOrFail($leadId, ['id']);
